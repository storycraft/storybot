...
class ChannelConnecter extends _storybotCore.CommandListener {
    constructor(main) {
        super();
        this.main = main;
    ...
    onMessage(message) {
        if (this.hasConnected(message.Source)) {
            if (message.Text != '') this.getConnected(message.Source).send(`${message.User.Name}: ${message.Text}`);
        }
    }
    ...
    removeQueue(queueCode) {
        if (!this.queueMap.has(queueCode)) return;
...
exports.default = ChannelConnecter;